...
class MovieTheaterController{
    ...
    public function addMT($name,$address,$numberOfCinemas,$capacityDefault,$priceDefault,$available = '1'){
        $MTs = $this->allMTs();
        if($address < 0 || $numberOfCinemas < 0 || $capacityDefault < 0 || $priceDefault < 0){
            $this->chargeAllAndBack('Negative value');
        }else{
